...
**2026-06-18 user directive (this session)**: Canvas replacement is now the active focus — replace tldraw 2D canvas engine + adapt surrounding UI/shell with the fabric-based "new interface" from E:\floorplan-react. Combine the 2D fabric canvas with the existing r3f 3D viewer. See docs/Handover.md § "2026-06-18 Session Target" for full scope and acceptance. "start dev" on the swap.
