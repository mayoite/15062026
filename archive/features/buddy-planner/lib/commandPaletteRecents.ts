 ...
 *
 *   - `buddycraft.commandPalette.recent`  → JSON string[] of action ids,
 ...
 *   - `buddycraft.commandPalette.scope`   → `'office' | 'all'`. Toggled by
...
/** Storage key for the recent-action ring. Mirrors the `buddycraft.*` prefix
 *  the rest of the app uses (see `filterPresetsStorage`, `useLibraryFavorites`). */
export const RECENTS_STORAGE_KEY = 'buddycraft.commandPalette.recent'

/** Storage key for the scope toggle. Same prefix convention. */
export const SCOPE_STORAGE_KEY = 'buddycraft.commandPalette.scope'
