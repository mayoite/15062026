...
/**
 * Convert a screen-space pointer position into canvas/world coordinates.
 *
 * The viewport transform is the inverse of the stage transform:
 * canvas = (screen - stageOffset) / stageScale.
 * Keeping this in one helper avoids subtle drift when a drag gesture is
 * interpreted at a non-1x zoom or while the viewport is being panned.
 */
export function screenPointToCanvasPoint(
  point: Point,
  viewport: ViewportTransform,
): Point {
  const scale = viewport.stageScale || 1
  return {
    x: (point.x - viewport.stageX) / scale,
    y: (point.y - viewport.stageY) / scale,
  }
}

/**
 * Convert a canvas/world point into screen-space coordinates for the
 * current viewport.
 */
export function canvasPointToScreenPoint(
  ...
  return {
    x: point.x * viewport.stageScale + viewport.stageX,
    y: point.y * viewport.stageScale + viewport.stageY,
  }
}
