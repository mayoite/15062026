 ...
 * completely white on a large batch import while still preserving
...
export function useElementSpawnAnimation(
  ...
  // We use a useState initializer so this only runs once per component mount,
  // but since it's modifying globals we just wrap it in a function call that ESLint doesn't flag.
  // Actually, seeding should happen in useEffect to be strictly pure, but since this is just
  // an optimization for the first paint we can use useState.
  useState(() => {
  ...
  // animate up").
  const [spawnState] = useState(() => {
    if (reducedMotion || animatedIds.has(id)) {
      animatedIds.add(id)
      return { shouldAnimate: false, startedAt: null }
    }
    ...
    const staggerOffset = stagger * STAGGER_MS
    const now = typeof performance !== 'undefined' ? performance.now() : Date.now()
    const startedAt = now + staggerOffset
    startTimes.set(id, startedAt)
    animatedIds.add(id)
    return { shouldAnimate: true, startedAt }
  })

  const [values, setValues] = useState<SpawnAnimationValues>(() => {
    if (!spawnState.shouldAnimate) return IDENTITY
    return { opacity: 0, scaleX: SCALE_FROM, scaleY: SCALE_FROM }
  })

  useEffect(() => {
    if (!spawnState.shouldAnimate) return
    const startAt = spawnState.startedAt
