...
import { appEnv, publicEnv } from '../lib/publicEnv'
 ...
 *
 * `next.config.ts` writes two coordinated artifacts on every production
 * build:
 *
 *   - The bundle gets `NEXT_PUBLIC_BUILD_ID` baked in via `env`.
 *   - `public/version.json` is written with the same `buildId`.
 *
 ...
 * old bundle's current build id. The mismatch is the "new version is
 ...
 * # Local dev
 *
 * In `next dev` the constants are still defined (config-load time runs
...
export interface UseBuildVersionOptions {
  ...
  intervalMs?: number
  /**
   * Ayush period after mount before the first poll fires. Prevents a
   ...
   * compile-time `NEXT_PUBLIC_BUILD_ID`. Tests inject a value directly.
   */
...
/**
 * Default for `enabled` — only poll in production builds.
 */
...
export function useBuildVersion(
  ...
  const {
    ...
    url = '/version.json',
    currentBuildId = publicEnv.buildId,
    scheduler,
    fetcher,
  } = options
