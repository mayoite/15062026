...
function HelpSearchPaletteInner({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg mx-4 bg-[color:var(--color-paper-raised)] dark:bg-gray-900 rounded-lg shadow-soft border border-[color:var(--color-paper-line)] dark:border-gray-800 overflow-hidden"
      ...
      >
        ...
        <ul className="max-h-80 overflow-y-auto py-1" role="listbox">
          {filtered.length === 0 ? (
            <li className="px-4 py-3 text-ui-13 text-gray-500 dark:text-gray-400">No matching sections.</li>
          ) : (
            filtered.map((s, i) => (
              <li
                key={s.id}
                ...
                onClick={() => jumpTo(s.id)}
                className={`px-4 py-2 text-ui-13 cursor-pointer ${
        ...
        <div className="border-t border-[color:var(--color-paper-line)] dark:border-gray-800 px-3 py-2 text-ui-11 text-gray-400 dark:text-gray-500 flex items-center gap-3">
          <span>
