    ...
    <span className="inline-flex items-center gap-1 whitespace-nowrap">
      {tokens.map((t, i) =>
        t.kind === 'key' ? (
          <kbd
            key={`${i}-${t.text}`}
            className="inline-flex items-center justify-center min-w-[1.5rem] h-6 px-1.5 rounded-md border border-[color:var(--color-paper-line)] dark:border-gray-700 bg-[color:var(--color-paper-sunken)] dark:bg-gray-800 text-[11px] font-medium font-mono text-gray-700 dark:text-gray-200 shadow-[0_1px_0_0_var(--surface-glass),inset_0_-1px_0_0_var(--surface-glass)] dark:shadow-[0_1px_0_0_var(--surface-glass),inset_0_-1px_0_0_var(--surface-glass)]"
          >
            {t.text}
          </kbd>
        ) : (
...
function OverlayContent({ setOpen }: { setOpen: (open: boolean) => void }) {
  ...
  // - Tab / Shift+Tab are trapped within dialog focus targets
  //
  // Bound to window (capture) so it still works if focus lands on any
  // child and so global shortcuts can't race it.
  useEffect(() => {
    const getFocusable = () => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      const activeInDialog = !!(active && dialogRef.current?.contains(active))
  ...
  return (
    ...
    >
      <div
        ref={dialogRef}
        className="bg-[color:var(--color-paper-raised)] dark:bg-gray-900 rounded-md shadow-soft p-6 max-w-3xl w-full mx-4 max-h-[85vh] overflow-y-auto"
      ...
      >
        ...
        >
          <input
            ...
            className="w-full px-3 py-2 text-ui-13 bg-[color:var(--color-paper-sunken)] dark:bg-gray-800 border border-[color:var(--color-paper-line)] dark:border-gray-700 rounded-md text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[color:var(--color-blueprint)] focus:border-transparent"
          />
        </form>

        <p className="text-ui-11 text-gray-500 dark:text-gray-400 mb-3" aria-live="polite">
        ...
        {filteredGroups.length === 0 ? (
          <div className="py-10 text-center text-ui-13 text-gray-500 dark:text-gray-400">
              ...
              <section key={group.title}>
                ...
                <ul className="flex flex-col gap-1.5">
                  {group.rows.map((row) => (
                    ...
                    >
                      <span className="text-ui-13 text-gray-700 dark:text-gray-200">{row.action}</span>
