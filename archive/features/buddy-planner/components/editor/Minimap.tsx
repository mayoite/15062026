...
import { coerceMinimapViewportSize } from '../../lib/minimapViewportSize'

const MINIMAP_WIDTH = 180
const MINIMAP_HEIGHT = 120
const COLLAPSED_SIZE = 40
const MINIMAP_ARIA_LABEL = 'Canvas minimap overview of floor content and current viewport'
...
function MinimapViewport({ bounds, minimapScale }: MinimapViewportProps) {
  ...
  const viewport = coerceMinimapViewportSize(
  ...
  const viewportW = (viewport.width / stageScale) * minimapScale
  const viewportH = (viewport.height / stageScale) * minimapScale
...
export function Minimap() {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    try {
      ...
      const v = localStorage.getItem('buddycraft.minimapCollapsed')
  ...
  useEffect(() => {
    try {
      localStorage.setItem('buddycraft.minimapCollapsed', collapsed ? '1' : '0')
    } catch {
      // ignore
    }
  }, [collapsed])
  ...
  const applyPan = useCallback(() => {
    ...
    const viewport = coerceMinimapViewportSize(
      { width: cs.stageWidth, height: cs.stageHeight },
      typeof window === 'undefined'
        ? null
        : { width: window.innerWidth, height: window.innerHeight },
    )
    ...
    setStagePosition(
      -canvasX * cs.stageScale + viewport.width / 2,
      -canvasY * cs.stageScale + viewport.height / 2,
    )
  ...
  if (collapsed) {
    return (
      <div
        ...
        className="absolute bottom-12 right-4 bg-[color:var(--color-paper-raised)] dark:bg-gray-900 border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded-lg shadow-soft overflow-hidden flex items-center justify-center"
  ...
  return (
    <div
      ...
      role="img"
      aria-label={MINIMAP_ARIA_LABEL}
      className="absolute bottom-12 right-4 bg-[color:var(--color-paper-raised)] dark:bg-gray-900 border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded-lg shadow-soft overflow-hidden select-none touch-none cursor-grab active:cursor-grabbing"
