import { useUIStore, type ImportIssue } from '../../stores/uiStore'
...
export function CSVImportSummaryModal() {
  ...
  const allIssues: Array<ImportIssue & { kind: 'skipped' | 'warning', reason?: string }> = [
  ...
  return (
    <Modal open onClose={handleDone} title="Import complete" size="lg">
      <ModalBody>
        <div className="flex gap-4 mb-4 text-ui-13">
        ...
        {allIssues.length > 0 && (
          <div className="max-h-60 overflow-y-auto border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded">
            <table className="w-full text-ui-11">
