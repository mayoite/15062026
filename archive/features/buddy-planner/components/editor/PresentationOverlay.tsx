 ...
 *
 * 3. A first-run hint toast: "Press Esc to exit", gated behind a localStorage flag so subsequent presentations do not repeat it. Auto-dismisses after 5s or on any keystroke.
 *
  ...
  if (reduceMotion) {
    ...
    return (
      <div aria-live="polite" className="contents">
        ...
        <button
          onClick={onExit}
          className="fixed top-4 right-4 z-[60] sr-only focus:not-sr-only focus:px-3 focus:py-2 focus:rounded-md focus:bg-gray-900 focus:text-white focus:text-ui-13 focus:font-medium focus:shadow-soft"
    ...
    <>
      ...
      */}
      <div
        ...
        className={`fixed bottom-4 right-4 z-[60] pointer-events-none px-3 py-1.5 rounded-full bg-gray-900/80 text-white text-ui-11 font-medium shadow-soft backdrop-blur-sm transition-opacity duration-300 ${
      ...
      */}
      <button
        onClick={onExit}
        className={`fixed top-4 right-4 z-[60] w-9 h-9 rounded-full bg-gray-900/80 hover:bg-gray-900 text-white text-base shadow-soft backdrop-blur-sm flex items-center justify-center transition-opacity duration-300 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-[color:var(--color-blueprint)] ${
...
const HINT_STORAGE_KEY = 'buddycraft.presentationHintSeen'

/**
 * One-shot tooltip pointing the operator at the presentation exit shortcut. Renders only on the very first time a user enters
 ...
 * pressing any key to continue also clears the hint).
 *
 ...
 * switcher is hidden, but the natural eye-line for guidance is still up there.
 */
function FirstRunHint() {
  ...
  return (
    <div
      role="status"
      className="fixed top-4 left-4 z-[60] px-3 py-2 rounded-md bg-[color:var(--color-blueprint-strong)] text-white text-ui-13 font-medium shadow-soft"
    >
      Press <kbd className="font-mono bg-white/20 px-1 rounded">Esc</kbd> to exit
    </div>
  )
}
