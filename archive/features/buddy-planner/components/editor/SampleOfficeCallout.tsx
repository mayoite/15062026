...
const dismissedKey = (officeId: string) => `buddycraft.sampleCalloutDismissed.${officeId}`
...
export function SampleOfficeCallout() {
  ...
  const isSample = (name: string | null) => name?.toLowerCase().includes('sample') ?? false
  if (!officeId || !isSample(officeName)) return null
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="absolute top-4 left-1/2 -translate-x-1/2 z-40 max-w-xl w-[calc(100%-2rem)] rounded-md border border-[color:var(--color-blueprint)]/40 bg-[color:var(--color-paper-raised)] dark:bg-gray-900 shadow-soft flex items-start gap-3 px-4 py-3"
    >
      <Sparkles
        size={18}
        ...
        aria-hidden="true"
      />
      <div className="flex-1 min-w-0 text-ui-13">
