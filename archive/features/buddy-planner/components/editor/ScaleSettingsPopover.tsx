...
export function ScaleSettingsPopover() {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className={`text-ui-11 px-2 py-1 rounded border ${
          open
      ...
      {open && (
        <div
          ...
          className="absolute top-full mt-1 right-0 z-40 w-64 bg-[color:var(--color-paper-raised)] dark:bg-gray-900 border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded-md shadow-soft p-3"
        >
          <h3 className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">
            Measurement
          </h3>

          <label className="block text-ui-11 text-gray-700 dark:text-gray-200 mb-1" htmlFor="scale-unit">
            Unit
          </label>
          <select
            id="scale-unit"
            ...
            className="w-full text-ui-13 border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded px-2 py-1 mb-3 focus:outline-none focus:border-[color:var(--color-blueprint)]"
          >
            {LENGTH_UNITS.map((u) => (
          ...
          <label className="block text-ui-11 text-gray-700 dark:text-gray-200 mb-1" htmlFor="scale-value">
            Scale: 1 canvas px ={' '}
          </label>
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-24 text-ui-13 border border-[color:var(--color-paper-line)] dark:border-gray-800 rounded px-2 py-1 focus:outline-none focus:border-[color:var(--color-blueprint)]"
              disabled={scaleUnit === 'px'}
            />
            <span className="text-ui-13 text-gray-500 dark:text-gray-400">{LENGTH_UNIT_SUFFIX[scaleUnit]}</span>
          </div>
