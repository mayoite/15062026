...
import {
  ...
  type LibraryItem,
} from '../../../data/elementLibraryCatalog'
import type {
  ElementType,
  TableType,
  TableElement,
...
// LibraryItem now lives in the catalog data module so the type and its data
// stay together. Re-exported here because several consumers
// (CanvasStage, LibraryPreview, useLibraryFavorites, useRecentLibraryItems,
// elementLibraryRecents) import it from this path.
 ...
 * library drags from member-assignment drags (which use
 ...
 * share the exact same factory Ã¢â‚¬â€ keep this pure so it can be called from
...
export function buildLibraryElement(
  ...
  zIndex: number,
  /**
   * Current floor's elements Ã¢â‚¬â€ passed in so the assignable-element
   * branches can hand out a sequential `deskId` ("1", "2", "3" Ã¢â‚¬Â¦).
...
// eslint-disable-next-line @typescript-eslint/consistent-type-imports
  ...
  const defaults = getDefaults(item.type, item.shape) || { width: 60, height: 60, fill: 'var(--surface-panel)', stroke: 'var(--border-soft)' }
  ...
  if (item.type === 'desk' || item.type === 'hot-desk') {
    // Generate a simple desk ID based on the number of desks already present.
    const deskCount = Object.values(existingElements).filter(el => el.type === 'desk' || el.type === 'hot-desk').length
    const deskId = `D-${deskCount + 1}`
  ...
  if (item.type === 'workstation') {
    const wsCount = Object.values(existingElements).filter(el => el.type === 'workstation').length
    const deskId = `WS-${wsCount + 1}`
    const positions = 4
  ...
  if (item.type === 'private-office') {
    const poCount = Object.values(existingElements).filter(el => el.type === 'private-office').length
    const deskId = `PO-${poCount + 1}`
...
interface LibraryTileProps {
  ...
  /** Present only for user-uploaded custom SVGs. Shows an "Ãƒâ€”" in place of
   ...
   *  (not tools) so this is normally false Ã¢â‚¬â€ but kept generic in case a
  ...
  /** Hover left or drag started Ã¢â‚¬â€ tear the tooltip down. */
...
function LibraryTile({
  ...
  return (
    <div
      ...
      // Hover lifts to a blue accent border + faint shadow Ã¢â‚¬â€ `motion-reduce`
...
interface LibrarySectionProps {
   ...
   *  caps at a small number, so showing "Recent Ã‚Â· 3" doesn't add much and
...
function LibrarySection({
  ...
  // per-category default Ã¢â‚¬â€ most sections start collapsed (see the hook)
  ...
  // Header copy: "Tables Ã‚Â· 4" Ã¢â‚¬â€ a middle-dot separator is the convention
...
export function ElementLibrary() {
  const libraryItems = getElementLibraryCatalog()
  const tileDescriptions = getElementLibraryDescriptions()
  const canEdit = useCan('editMap')
  const canAnnotate = canEdit
  ...
  // The legacy zustand `useRecentLibraryItems` store stays in place Ã¢â‚¬â€ it's
  // also written to from CanvasStage's drop handler (out of scope for this
  // wave) Ã¢â‚¬â€ and we mirror our writes there so both lists stay in sync.
  ...
  // around Ã¢â‚¬â€ see `dragInProgress` checks below.
  ...
  // Ã¢â‚¬â€ anything that bubbles up to window therefore happened "outside"
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!shape) {
        setUploadError('Library full Ã¢â‚¬â€ delete a custom shape first.')
        return
      }
      setUploadError(null)
    } catch {
  ...
  // We bump the recents list at drag-start (rather than drop) Ã¢â‚¬â€ detecting
  ...
  const categories = useMemo(
    () => [...new Set(libraryItems.map((i) => i.category))],
    [libraryItems],
  )
  ...
  // matching is a no-op for now Ã¢â‚¬â€ if we later add tags, extend this here.
  ...
  const filtered = useMemo(
    () =>
      isSearching
        ? [...libraryItems, ...customShapeItems].filter(matchesQuery)
        : [],
    [isSearching, libraryItems, customShapeItems, matchesQuery],
  )
  const favoriteItems = useMemo(
    () => libraryItems.filter((i) => favoriteSet.has(favoriteKey(i))),
    [libraryItems, favoriteSet],
  )

  // Viewers can't place anything Ã¢â‚¬â€ the library is inert for them. Hiding
  ...
  // Most library items are factories (not tools) so this is normally false,
  ...
  return (
    ...
    <div className="p-3 pb-6 relative">
      {/* Search input. The Wave 19A polish swaps the bare <input> for the
          ...
          match the rest of the app, and adds a leading Search icon Ã¢â‚¬â€ small
          ...
          palette instead Ã¢â‚¬â€ which itself can navigate to the library. */}
      <div className="mb-3 rounded-xl border border-[color:var(--color-paper-line)] bg-[color:var(--color-paper-sunken)]/80 px-3 py-2 dark:border-gray-800 dark:bg-gray-900/50">
        ...
        <p className="mt-1 text-[11px] leading-5 text-gray-600 dark:text-gray-300">
          Build geometry first, then place inventory, then review annotations and export.
        </p>
      </div>
      ...
      {/* Recent row Ã¢â‚¬â€ always visible when populated. Search ignores it so
      ...
      {/* Defensive empty-catalog state. The catalog can be tenant-driven, so
          the section is here to degrade gracefully when no entries are active:
      ...
      {libraryItems.length === 0 && customShapeItems.length === 0 ? (
        <div
          role="status"
        ...
        filtered.length === 0 ? (
          ...
          // which token came back empty Ã¢â‚¬â€ typos are the most common cause.
          <div
            role="status"
          ...
          <>
            ...
            {categories.map((cat, idx) => {
              const matched = libraryItems.filter(
      ...
      {/* Single shared hover tooltip Ã¢â‚¬â€ see HoverTooltip below. Rendered via
 ...
 *
 *   1. The sidebar container is `overflow-hidden` Ã¢â‚¬â€ without a portal, the
 ...
 * compute this once per render Ã¢â‚¬â€ re-running on scroll would be more
 * accurate but the dwell timer is short and tooltips are torn down on
 * mouse-leave anyway, so a static placement is fine.
 */
function HoverTooltip({ item, rect, tileDescriptions }: HoverTooltipProps) {
  // Approx height: 1 line title + 1-2 lines description + 1 line hint.
  // Doesn't have to be exact Ã¢â‚¬â€ used only to choose above-vs-below.
  ...
  // the right viewport edge so it never cuts off Ã¢â‚¬â€ the sidebar is on the
  ...
  // Skip the fade-in for reduced-motion users Ã¢â‚¬â€ the tooltip just appears.
  ...
  return createPortal(
    ...
    >
      ...
      <div className="opacity-80 leading-snug">
        {tileDescriptions[tileKey(item)] ?? `${item.category} element.`}
      </div>
      <div className="mt-1 text-[10px] opacity-60">
        Drag onto canvas, or click to add at viewport centre.
      </div>
      {/* Triangle pointer Ã¢â‚¬â€ a 1Ãƒâ€”1 rotated square sitting on the appropriate
