 ...
 *     rounded-md card, hairline border, soft 2xl shadow, dark-mode aware.
...
function CommandPaletteBody() {
  ...
  const announceGuestBlocked = useGuestAccessStore((s) => s.announceBlocked)
  ...
  // the pattern in CalibrateScaleModal.
  useEffect(() => {
    registerModalOpen()
  ...
  // Empty state shows when the list has nothing to render.
  ...
  return (
    ...
    >
      <div
        className="bg-[color:var(--color-paper-raised)] dark:bg-gray-900 rounded-md shadow-soft border border-[color:var(--color-paper-line)] dark:border-gray-800 w-full max-w-[600px] mx-4 overflow-hidden"
          ...
          {showEmptyState && (
            ...
            >
              <SearchX
                ...
                aria-hidden="true"
              />
              <div className="text-ui-13 text-gray-600 dark:text-gray-300">
                No commands match
              </div>
...
function PaletteRow({
  ...
  return (
    <button
      ...
      onClick={onActivate}
      className={`w-full text-left px-4 py-2 text-ui-13 flex items-center gap-2 ${active
    ...
    >
      ...
      {item.subtitle && (
        <span className="text-ui-11 text-gray-400 dark:text-gray-500 flex-shrink-0">
          {item.subtitle}
        </span>
      )}
      {shortcut && (
