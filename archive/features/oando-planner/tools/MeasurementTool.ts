...
export class MeasurementUtils {
  ...
  createLinearDimension(startPoint: Vec, endPoint: Vec): string {
    ...
    this.editor.createShape(dimensionShape as unknown);
  ...
  createAngularDimension(vertex: Vec, startPoint: Vec, endPoint: Vec): string {
    ...
    if (this.options.showLabels) {
      ...
      this.editor.createShape(labelShape as unknown);
    }
  ...
  measureDistanceBetweenShapes(shape1Id: string, shape2Id: string): number {
    const shape1 = this.editor.getShape(shape1Id as unknown);
    const shape2 = this.editor.getShape(shape2Id as unknown);
  ...
  measureRoomFromWalls(wallIds: string[]): { width: string; length: string; area: string } {
    const walls = wallIds
      .map(id => this.editor.getShape(id as unknown))
      .filter(w => w !== null) as TLShape[];
  ...
  clearMeasurements() {
    ...
    for (const shape of allShapes) {
      if (shape.type === "planner-measurement") {
        this.editor.deleteShape(shape.id as unknown);
        continue;
      }
