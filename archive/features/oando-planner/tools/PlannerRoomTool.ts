...
class PlannerRoomToolDrawing extends StateNode {
  ...
  override onPointerMove() {
    ...
    const widthMm = Math.max(1, Math.abs(dx));
    const heightMm = Math.max(1, Math.abs(dy));
    ...
    this.editor.updateShape({
      ...
      props: {
        ...
        widthMm,
        heightMm,
        areaSqm: (widthMm * heightMm) * 0.0001,
        perimeterMm: Math.round((widthMm + heightMm) * 2 * 10),
      },
    });
  }
        ...
        if (w < 4 && h < 4) {
          ...
          this.editor.updateShape({
            ...
            props: {
              ...
              widthMm: size,
              heightMm: size * 0.75,
              areaSqm: (size * size * 0.75) * 0.0001,
              perimeterMm: Math.round((size + size * 0.75) * 2 * 10),
            }
          });
        }
      }
