  ...
  private createRoomLabels(rooms: DetectedRoom[]) {
    for (const room of rooms) {
      ...
      const duplicate = allShapes.some(s => {
        ...
        const sCenter = new Vec(s.x + (s.props as unknown).widthMm / 2, s.y + (s.props as unknown).heightMm / 2);
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  updateRoomLabel(roomId: string, newName: string) {
    const shape = this.editor.getShape(roomId as unknown);
    if (shape && shape.type === "planner-room") {
      this.editor.updateShape({
        id: roomId as unknown,
        type: "planner-room",
        props: {
          label: newName
        }
