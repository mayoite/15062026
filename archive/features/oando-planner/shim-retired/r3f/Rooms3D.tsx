...
import { usePlannerStore, type Room } from "@/features/planner/store/plannerStore";
...
export function Rooms3D() {
  const rooms = usePlannerStore((s: { rooms: Room[] }) => s.rooms);
  const selectedId = usePlannerStore((s: { selectedId: string | null }) => s.selectedId);
  const setSelected = usePlannerStore((s: { setSelected: (id: string | null) => void }) => s.setSelected);
  const tool = usePlannerStore((s: { tool: string }) => s.tool);
  const deleteItem = usePlannerStore((s: { deleteItem: (id: string) => void }) => s.deleteItem);
