// @ts-nocheck
import type { QuoteCartItem } from "@/stores/useQuoteCartStore";

import type { BoqItem } from "../ui/types";
...
export function buildPlannerQuoteCartItems(items: BoqItem[]): QuoteCartItem[] {
  return (Object.entries(groupPlannerBoqItems(items)) as Array<[string, GroupedPlannerBoqItem]>).map(
    ([groupKey, item]) => ({
      id: `planner-${groupKey}`,
      name: item.name,
      qty: item.qty,
