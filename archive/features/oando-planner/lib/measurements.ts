...
import type { PlannerShapeMeta } from "@/features/planner/shared/catalog/types";
...
function parseMetricMeasurementInput(value: string) {
  ...
  // Detect an explicit metric unit suffix; canonical storage is millimeters.
  const unitMatch = trimmed.match(/(mm|cm|m)$/i);
  const unit = unitMatch ? unitMatch[1].toLowerCase() : "mm";
  const numericPart = unitMatch ? trimmed.slice(0, -unitMatch[1].length).trim() : trimmed;

  const parsed = Number.parseFloat(numericPart);
  if (!Number.isFinite(parsed) || parsed <= 0) return null;

  const mmPerUnit = unit === "m" ? 1000 : unit === "cm" ? 10 : 1;
  return Math.round(parsed * mmPerUnit);
}
...
export function parseMeasurementInput(
  ...
  return unitSystem === "ft-in"
    ? parseFeetAndInchesInput(value)
    : parseMetricMeasurementInput(value);
}

export function formatMetricFromBounds(
...
export function getMetricLabelForShape(
  editor: Editor,
...
export function deriveViewportState(
  ...
  const selectionBounds =
    selectionIds.length > 0 ? editor.getSelectionPageBounds() : null;
  ...
  const selectionShape =
    selectionIds.length === 1 ? editor.getShape(selectionIds[0]) : null;
  ...
  return {
    ...
    roomMetrics: mergedStructuralBounds
      ? formatMetricFromBounds(mergedStructuralBounds, unitSystem)
      : "No room shell yet",
