...
export function runPlannerComplianceCheck(
  ...
  const plannerShapes = shapes.filter(
    (shape) => getShapeMeta(shape.meta).isPlannerItem,
  );
  let overlapCount = 0;
  ...
  for (let index = 0; index < plannerShapes.length; index += 1) {
    for (
      ...
      nextIndex < plannerShapes.length;
      nextIndex += 1
    ) {
      ...
      const clearanceX = Math.max(
        0,
        Math.max(boundsA.minX - boundsB.maxX, boundsB.minX - boundsA.maxX),
      );
      const clearanceY = Math.max(
        0,
        Math.max(boundsA.minY - boundsB.maxY, boundsB.minY - boundsA.maxY),
      );
      const distance = Math.sqrt(
        clearanceX * clearanceX + clearanceY * clearanceY,
      );
  ...
  if (overlapCount > 0) {
    warnings.push(
      `CRITICAL: ${overlapCount} workstation(s) are severely overlapping.`,
    );
  }
