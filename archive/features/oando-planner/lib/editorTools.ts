...
function snapPagePointToEndpoints(
    ...
    props: {
      ...
      points: {
        ...
        [indices[1]]: {
          id: indices[1],
          index: indices[1],
          x: toPageUnits(DEFAULT_WALL_SEGMENT_MM),
          y: 0,
        },
      },
    },
...
export function resolvePlannerWallJoins(
  editor: Editor,
  shapeIds?: PlannerShapeId[],
) {
  const targets = editor
    ...
    .filter((shape): shape is PlannerLineShape => {
      ...
      return (
        ...
        !meta.isPlannerItem &&
        !meta.isRoomDimension &&
        (!shapeIds || shapeIds.includes(shape.id))
      );
    });

  targets.forEach((shape) => {
    const endpoints = collectWallEndpoints(editor, shape.id);
    const normalizedPoints = normalizeLinePoints(
      getSortedLinePoints(shape),
    ).map((point) => {
      const snapped = snapPagePointToEndpoints(
...
export function readPlannerSelectionDimensions(
  ...
  if (
    ...
    return {
      shapeId: shape.id,
      shapeName:
        meta.text || (shape.type === "image" ? "Placed item" : "Planner shape"),
      mode: "box",
      widthMm,
      heightMm,
    };
...
export function updatePlannerSelectionDimensions(
  ...
  if (
    ...
    const currentWidthMm = normalizePositiveMillimeters(
      toMillimeters(shape.props.w),
    );
    const currentHeightMm = normalizePositiveMillimeters(
      toMillimeters(shape.props.h),
    );
    if (currentWidthMm === null || currentHeightMm === null) return false;

    const widthMm =
      typeof next.widthMm === "undefined"
        ? currentWidthMm
        : normalizePositiveMillimeters(next.widthMm);
    const heightMm =
