...
function EditableMeasurementLabel({
  ...
  const { startX, startY, endX, endY, lengthMm, fontSize, textColor } = shape.props;
    ...
    <g transform={`translate(${x}, ${y}) rotate(${angleDeg})`}>
      ...
      <text
        ...
        onDoubleClick={(event) => {
          ...
          const next = window.prompt("Measurement length in meters", (lengthMm / 1000).toFixed(2));
          if (!next) return;
...
export class PlannerMeasurementShapeUtil extends ShapeUtil<PlannerMeasurementTLShape> {
  ...
  component(shape: PlannerMeasurementTLShape) {
    const { startX, startY, endX, endY, lengthMm, lineColor, strokeWidth, showValue } = shape.props;
