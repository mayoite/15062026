...
export class PlannerDoorShapeUtil extends ShapeUtil<PlannerDoorTLShape> {
  ...
  getGeometry(shape: PlannerDoorTLShape) {
    ...
    const h = Math.max(1, shape.props.thicknessMm);
  ...
  component(shape: PlannerDoorTLShape) {
    const w = Math.max(1, shape.props.widthMm);
    const h = Math.max(1, shape.props.thicknessMm);
    const { swingDirection, swingAngle, showSwingArc, showDoorPanel, frameColor, panelColor, strokeWidth } = shape.props;
    ...
    const arcPath = `M ${w} 0 A ${w} ${w} 0 0 ${sweepFlag} ${px} ${py}`;

    return (
      <SVGContainer>
        <g stroke={frameColor || "var(--color-primary)"} strokeWidth={strokeWidth ?? 2} fill="none">
          {/* Main frame / wall opening line */}
          <line x1={0} y1={h / 2} x2={w} y2={h / 2} strokeDasharray="4 4" opacity={0.5} />
