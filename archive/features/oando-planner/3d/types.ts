...
interface LegacyPlannerSceneEnvelope {
...
function isPlannerSceneEnvelope(
  value: unknown,
): value is LegacyPlannerSceneEnvelope {
  if (!value || typeof value !== "object") return false;
  const c = value as Partial<LegacyPlannerSceneEnvelope>;
  return c.type === "cad-suite-planner-scene" && c.version === 1;
}

function getPlannerSceneEnvelope(
  sceneJson: PlannerJsonValue,
): LegacyPlannerSceneEnvelope | null {
  ...
  if (sceneJson && typeof sceneJson === "object" && !Array.isArray(sceneJson)) {
    ...
    if (isPlannerSceneEnvelope(obj.plannerScene)) return obj.plannerScene;
  }
  return null;
}
...
function resolveDocumentRoomFallback(
  document: unknown,
  normalized: PlannerDocument,
) {
  const candidate =
    document && typeof document === "object"
      ? (document as Partial<PlannerDocument>)
      : null;
...
function buildSafeRoom(
  ...
  return {
    ...
    wallThicknessMm: coercePositiveMm(
      room?.wallThicknessMm,
      DEFAULT_WALL_THICKNESS_MM,
    ),
    floorThicknessMm: coercePositiveMm(
      room?.floorThicknessMm,
    ...
    return [
      {
        ...
        sizeMm: {
          widthMm: coercePositiveMm(
            item.sizeMm?.widthMm,
            DEFAULT_ITEM_WIDTH_MM,
          ),
          depthMm: coercePositiveMm(
            item.sizeMm?.depthMm,
            DEFAULT_ITEM_DEPTH_MM,
          ),
          heightMm: coercePositiveMm(
            item.sizeMm?.heightMm,
            DEFAULT_ITEM_HEIGHT_MM,
          ),
        },
...
function resolveFallbackColor(category: string) {
  ...
  if (normalized.includes("storage")) return "var(--border-soft)";
  if (
  ...
  if (normalized.includes("table")) return "var(--border-soft)";
  if (normalized.includes("desk") || normalized.includes("work"))
    return "var(--border-soft)";
  return "var(--border-soft)";
}

export function buildPlanner3DSceneDocument(
  ...
  const normalizedId = typeof normalizedRecord.id === "string" ? normalizedRecord.id : null;
  const documentId = typeof documentRecord.id === "string" ? documentRecord.id : null;
  const normalizedName =
    typeof normalizedRecord.name === "string" ? normalizedRecord.name : null;

  return validatePlanner3DSceneDocument({
    type: "cad-suite-planner-3d-scene",
    version: 1,
    id: normalizedId ?? documentId ?? "planner-preview",
    title: normalized.title ?? normalizedName ?? "Untitled plan",
...
export function summarizePlannerDocument(
  document: PlannerDocument,
  ...
  const largestItem = scene.items.reduce<{
    name: string;
    areaMm2: number;
  } | null>((best, item) => {
