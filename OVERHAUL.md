...
**Scope:** Overhaul the live planner at `/planner/guest/` and `/planner/canvas/` — not the marketing pages, not admin, not a blueprint revival.
...
**Problem:** `StartingPointStep` copy says *“We'll generate a basic room outline based on your {sq ft}”* but only calls `onComplete()`. `RoomPresetsOnOpen` is `return null`. Meanwhile `suggestLayoutGridPack()` in `ai/spaceSuggest.ts` already computes `walls`, `room`, and `furniture` from setup metadata — but `applySuggestedLayout()` **drops the walls array** and calls `placeCatalogItem()` **without coordinates**.
...
| File | Action |
|---|---|
| `onboarding/StartingPointStep.tsx` | On complete: call new `bootstrapGuestShell(metadata)` before `onComplete` |
| `onboarding/projectSetup.ts` | Export `metadataToSpaceSuggestInput()` |
| `ai/spaceSuggest.ts` | Add `buildShellOnlyLayout(metadata)` — walls + room, no furniture (thin wrapper over `estimateRoomMm` + `buildPerimeterWalls`) |
...
Do **not** add blueprint import back. Remove `floorPlanImport` references from onboarding/marketing when copy still implies it.
...
**Out of scope unless product asks:** `supabaseSync`, `offlineMode`, i18n extraction, panorama export.
