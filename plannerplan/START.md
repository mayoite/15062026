...
|---|---|---|---|
| B1 | Starter shell is queued, but wall-backed shell generation is incomplete | `StartingPointStep` now seeds bootstrap layout; `applySuggestedLayout` still ignores `layout.walls` | WS2 |
...
`PHASE-01` … `PHASE-08` are historical only. Do not execute phase tasks that mention blueprint import, calibration, PDF trace, or the removed Rooms tab.
