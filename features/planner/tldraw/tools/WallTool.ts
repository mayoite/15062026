...
import { getWallThicknessCanvasUnits } from "../shapes/WallShape";
import { getEditorSnapThreshold, snapEditorPointOrGrid, snapWallEndpoint } from "./tldrawSnap";
...
export class WallDrawingUtils {
  ...
  private createWall(startPoint: Vec, endPoint: Vec) {
    ...
    const lengthMm = canvasUnitsToMillimeters(Math.hypot(dx, dy));
  ...
  setMaterial(material: WallMaterial) {
    ...
    this.state.thickness = getWallThicknessCanvasUnits(material);
  }
...
class PlannerWallToolDrawing extends StateNode {
  ...
  override onPointerMove() {
    ...
    const snapStart = this.applySnapping(this.startPoint);
    // Use angle-constrained snap when Shift is held (0/45/90 degree increments)
    const shiftKey = this.editor.inputs.shiftKey;
    const snapResult = snapWallEndpoint(this.editor, snapStart, currentPoint, shiftKey, this.wallId);
    const snapEnd = snapResult.point;
...
// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      ...
      props: {
        endX: dx,
        endY: dy,
        lengthMm: canvasUnitsToMillimeters(length),
      },
    });
  }
...
// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    ...
    if (shape && shape.type === "planner-wall") {
      ...
      const dx = wallShape.props.endX;
      const dy = wallShape.props.endY;
      const length = Math.hypot(dx, dy);
  ...
  private getJunction(point: Vec, excludeId?: string): { hasJunction: boolean; type?: "L" | "T" | "cross" } {
    ...
    for (const shape of allShapes) {
      if (shape.type === "planner-wall" && shape.id !== excludeId) {
        const wallShape = shape as PlannerWallTLShape;
        const start = new Vec(shape.x + wallShape.props.startX, shape.y + wallShape.props.startY);
        const end = new Vec(shape.x + wallShape.props.endX, shape.y + wallShape.props.endY);
