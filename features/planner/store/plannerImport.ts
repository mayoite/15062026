...
export function parsePlannerDocumentImportText(
  text: string,
  ...
  try {
    ...
    return {
      ok: false,
      errors: [
        error instanceof Error ? error.message : "Invalid planner import JSON.",
      ],
    };
  }
}

export async function parsePlannerDocumentImportFile(
  file: File,
): Promise<PlannerDocumentImportResult> {
  return parsePlannerDocumentImportText(await file.text());
}

export function parsePlannerDocumentImportValue(
  value: unknown,
): PlannerDocumentImportResult {
  return parsePlannerDocumentImport(value);
}

export function validatePlannerDocumentImportText(
  text: string,
...
export function validatePlannerDocumentImportValue(
  value: unknown,
): PlannerImportValidationResult {
