...
export async function listPlannerManagedProductsForPlannerCatalog(): Promise<
...
> {
  ...
  if (error) {
    ...
    console.error(
      "[planner] Unable to load planner-managed products:",
      error.message,
    );
    return [];
  }
...
export async function upsertPlannerManagedProduct(
  ...
  if (error || !data) {
    throw new Error(
      `Unable to save planner-managed product: ${error?.message ?? "Unknown error"}`,
    );
  }
...
export async function deletePlannerManagedProduct(
  client: SupabaseClient,
  id: string,
): Promise<boolean> {
  const { data, error } = await client
  ...
  if (error) {
    throw new Error(
      `Unable to delete planner-managed product: ${error.message}`,
    );
  }
