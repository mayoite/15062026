...
export type PlannerDraftLoadStatus =
  | "loaded"
  | "missing"
  | "expired"
  | "invalid"
...
function normalizePlannerDraftScope(
...
function isDraftEnvelope(
  value: unknown,
): value is Partial<PlannerDraftEnvelope> {
  return Boolean(value && typeof value === "object" && "document" in value);
}

function resolveDraftExpiryMs(
  envelope: Partial<PlannerDraftEnvelope>,
): number | null {
...
function isExpiredDraftEnvelope(
  envelope: Partial<PlannerDraftEnvelope>,
  nowMs = Date.now(),
): boolean {
...
function buildPlannerDraftLoadScopes(
  scope: PlannerDraftScope = {},
): PlannerDraftScope[] {
...
function loadPlannerDraftDocumentAtKey(
  ...
  try {
    ...
    if (isDraftEnvelope(parsed)) {
      ...
      return {
        document: plannerDocumentSchema.parse(
          normalizePlannerDocument(parsed.document),
        ),
        status: "loaded",
        scope,
      };
...
export function cleanupExpiredPlannerDrafts(
...
export function resolvePlannerDraftDocument(
  scope: PlannerDraftScope = {},
): PlannerDraftLoadResult {
...
export function savePlannerDraftDocument(
  ...
  const normalized = plannerDocumentSchema.parse(
    normalizePlannerDocument(document),
  );
...
export function loadPlannerDraftDocument(
  scope: PlannerDraftScope = {},
): PlannerDocument | null {
  return resolvePlannerDraftDocument(scope).document;
}

export function deletePlannerDraftDocument(
  scope: PlannerDraftScope = {},
): boolean {
  const storage = getLocalStorage();
