...
function readPlannerManagedProductObject(
  value: unknown,
...
function resolvePlannerManagedProductActiveState(
...
export function plannerManagedProductRowToCatalogProduct(
  row: PlannerManagedProductRow,
  ...
  return {
    ...
    detailedInfo: {
      ...
      features: [],
      dimensions:
        typeof row.specs.dimensions === "string" ? row.specs.dimensions : "",
      materials: Array.isArray(row.specs.materials)
        ? row.specs.materials.filter(
